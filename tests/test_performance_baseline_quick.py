...
def test_five_turn_latency(daemon_sock: str) -> tuple:
    ...
    for turn in range(1, 6):
        ...
        result = rpc_call(daemon_sock, "generate", {
            "session_id": session_id,
            "user_prompt": USER_PROMPT,
        }, req_id=str(turn + 1), timeout=600.0).get("result", {})
        dt = time.time() - t0
...
def test_ten_session_memory(svc: subprocess.Popen, daemon_proc: subprocess.Popen, daemon_sock: str) -> int:
    print("\n--- 5-session shared system prompt memory (quick mode) ---")
    for i in range(5):
...
def main() -> int:
    ...
    print(f"5-turn latencies (32 tokens/turn): {[f'{x:.2f}s' for x in latencies]}")
    print(f"5-session daemon RSS: {rss_kb / 1024:.1f} MiB")
    print(f"service restart: {startup_dt:.2f}s, post-restart turn: {post_dt:.2f}s")

    ok = True
    if latencies[4] > 10.0:
        print(f"INFO: turn5 latency {latencies[4]:.2f}s (quick mode, 32 tokens)")
    if rss_kb > 1_500_000:
